...
class Client extends EE {
	 ...
	 * @param {Object} arg 
	 */
	async function createServer(arg) {
...
class Server {
	 ...
	 */
	async function createChannel(arg) {
...
class Channel {
	...
	async function getMessages(arg) {
	 ...
	 */
	async function createInvite(arg) {
		var payload = {
			max_age: 0,
			max_users: 0,
			temporary: false
...
class Member {
	 ...
	 */
	async function moveTo(voiceChannel) {
	 ...
	 * @param {String} nick 
	 */
	async function editNickname(nick) {
...
class Message {
	 ...
	 * @param {Emoji|String} emoji 
	 */
	async function getReaction(emoji) {
		if (!emoji) throw new TypeError("Emoji data expected");
